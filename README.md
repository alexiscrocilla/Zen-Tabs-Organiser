...
- **An identity check on `event.target` makes everything but that one element inert.** Firefox toggles a group from `on_click` only when `event.target === this.#labelElement` — not a descendant, not the container. So a header this mod paints across the whole row answered clicks on the title alone: the icon beside it and the tinted band behind both did nothing, and the band is the largest target of the three. The fix claims the whole header in the capture phase and stops the event there, so the native handler cannot also fire and toggle it back.
